...
# ─────────────────────────────────────────────
# This is the FIRE / EMS service. It is intentionally a fully separate
# process from the police app.py — separate env vars, separate Supabase
# tables, separate storage prefix, separate in-memory state. Nothing here
# reads or writes anything the police service touches.
# ─────────────────────────────────────────────

clients = []  # SSE queues for this service only
...
MAX_INCIDENTS  = 5000
MAX_LOG_ROWS   = 5000
AUDIO_BUCKET   = "audio-clips"     # same bucket as police, different prefix below
AUDIO_PREFIX   = "fire_clips"      # keeps fire audio physically separate from police "clips/"

INCIDENT_TABLE = "fire_incidents"
LOG_TABLE      = "fire_radio_log"

# Repeated-transmission de-dupe: fire/EMS dispatch conventionally reads a
# call out twice. We don't want that to become two incident cards, so we
# keep a short rolling memory of recent call text and skip creating a new
# incident if something very similar just came through.
DEDUP_WINDOW_SECONDS      = 300
DEDUP_SIMILARITY_THRESHOLD = 0.55
_recent_calls = []  # list of (timestamp, normalized_text) — capped by prune below
_recent_calls_lock = threading.Lock()

EASTERN     = ZoneInfo("America/New_York")
groq_client = Groq(api_key=GROQ_API_KEY_BACKUP)
...
def get_incidents():
    ...
    res = (get_db().table(INCIDENT_TABLE)
           .select("*")
...
def broadcast(kind: str, data: dict):
...
def get_stats():
    try:
        ...
        all_time = db.table(INCIDENT_TABLE).select("id", count="exact").execute().count or 0

        total = (db.table(INCIDENT_TABLE)
                 .select("id", count="exact")
                 .gte("created_at", today_start_utc)
                 .execute()).count or 0

        high = (db.table(INCIDENT_TABLE)
                .select("id", count="exact")
                .gte("created_at", today_start_utc)
                .eq("priority", "High")
                .execute()).count or 0

        rows = (db.table(INCIDENT_TABLE)
...
# ─────────────────────────────────────────────
# Capture — identical curl-subprocess approach to the police service, just
# pointed at STREAM_URL2. Kept as its own copy so nothing shared can break
# the police pipeline if this file is edited later.
# ─────────────────────────────────────────────
...
def get_duration(audio_bytes: bytes) -> float:
    ...
    try:
        ...
        print(f"[fire-capture] ffprobe duration check failed: {e}", flush=True)
        return -1.0
    finally:
        os.unlink(path)
...
def convert_to_mp3(ts_bytes: bytes) -> bytes | None:
    ...
    if result.returncode != 0 or not os.path.exists(out_path) or os.path.getsize(out_path) < 1000:
        ...
        print(f"[fire-capture] ffmpeg convert failed: {err}", flush=True)
...
def trim_silence(audio_bytes: bytes) -> bytes:
    try:
        ...
        if result.returncode != 0 or not os.path.exists(out_path):
            print("[fire] ffmpeg failed, using original audio", flush=True)
            return audio_bytes
        ...
        if len(trimmed) < 1000:
            print("[fire] Trim: pure silence detected, skipping", flush=True)
            return b""

        print(f"[fire] Trim: {len(audio_bytes)//1024}KB → {len(trimmed)//1024}KB", flush=True)
        return trimmed

    except Exception as e:
        print(f"[fire] Trim error: {e}", flush=True)
        return audio_bytes
...
def upload_audio(audio_bytes: bytes) -> str | None:
    try:
        ...
        path = f"{AUDIO_PREFIX}/clip_{ts}.mp3"
...
def delete_audio(audio_url: str):
    try:
        ...
        print(f"[fire] Audio delete error: {e}", flush=True)
...
PARSE_PROMPT = """You are a fire/EMS dispatch parser for Amherst NY (Erie County).
...
def is_duplicate_call(transcript: str) -> bool:
    """True if this transcript is a near-repeat of a call we heard in the
    last DEDUP_WINDOW_SECONDS — i.e. the second time dispatch reads out the
    same call. Keeps us from double-carding one incident."""
    now = time.time()
    norm = _normalize(transcript)
    with _recent_calls_lock:
        global _recent_calls
        _recent_calls = [(t, txt) for (t, txt) in _recent_calls if now - t < DEDUP_WINDOW_SECONDS]
        for (_, txt) in _recent_calls:
            if difflib.SequenceMatcher(None, norm, txt).ratio() >= DEDUP_SIMILARITY_THRESHOLD:
                return True
    return False


def remember_call(transcript: str):
    with _recent_calls_lock:
        _recent_calls.append((time.time(), _normalize(transcript)))


def purge_old_incidents():
    try:
        db    = get_db()
        total = db.table(INCIDENT_TABLE).select("id", count="exact").execute().count or 0
        if total <= MAX_INCIDENTS:
            return
        oldest = (db.table(INCIDENT_TABLE)
                  .select("id, audio_url")
        ...
        for row in oldest:
            ...
            db.table(INCIDENT_TABLE).delete().eq("id", row["id"]).execute()
...
def purge_old_log_rows():
    try:
        db    = get_db()
        total = db.table(LOG_TABLE).select("id", count="exact").execute().count or 0
        if total <= MAX_LOG_ROWS:
            return
        oldest = (db.table(LOG_TABLE)
                  .select("id")
                  .order("created_at", desc=False)
                  .limit(total - MAX_LOG_ROWS)
                  .execute()).data or []
        for row in oldest:
            db.table(LOG_TABLE).delete().eq("id", row["id"]).execute()
    except Exception as e:
        print(f"[fire] Log purge error: {e}", flush=True)


def save_log_entry(transcript: str):
    ...
    This is intentionally decoupled from incident detection."""
    try:
        db  = get_db()
        row = {
            "text":       transcript,
        ...
        res   = db.table(LOG_TABLE).insert(row).execute()
...
def save_incident(parsed: dict, transcript: str, audio_url: str | None):
    try:
        ...
        res   = db.table(INCIDENT_TABLE).insert(row).execute()
        saved = res.data[0] if res.data else row
        broadcast("incident", saved)
...
def process_audio_chunk(audio: bytes):
    try:
        ...
        # traffic, real dispatch calls. This powers the flat radio log tab.
        save_log_entry(transcript)

        # Fresh-calls-only gate #1: is this just a repeat of a call that
        # was already read out in the last few minutes?
        if is_duplicate_call(transcript):
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5001))  # different default port than the police service
